...
public interface AppUserRegistrationValidator extends Function<RegistrationRequest, AppUserRegistrationValidator.ValidationResult> {
    ...
    static AppUserRegistrationValidator isName(){
        return request -> new NameValidator().test(request.getUserName())  ? ValidationResult.SUCCESS : ValidationResult.EMAIL_NOT_VALID;
    }
    ...
    static AppUserRegistrationValidator isPasswordValid(){
        return request -> new PasswordValidator().test(request.getPassword())? ValidationResult.SUCCESS : ValidationResult.NID_NOT_VALID;
    }
